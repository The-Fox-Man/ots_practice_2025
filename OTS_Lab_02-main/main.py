...
def perform_switch_case(state, t, turn):
    ...
    if state == "DOWN":
        ...
        if y <= -(num_turns - turn): #Изменено вручную
            state = "LEFT"
    ...
    if state == "RIGHT":
        t.forward(10)  # Перемещение
        if turn > num_turns - num_turns/2: #Изменено вручную
            state = "STOP"
            return state, turn
        if x >= num_turns - turn: #Изменено вручную
            state = "DOWN"
